...
class User82Controller extends Controller
{
    public function loginView82()
    {
    ...
    public function registerView82()
    {
    ...
    public function lupaPasswordView82()
    {
    ...
    public function lupaPassword82(Request $request)
    {
    ...
    public function tambahAgama82(Request $request)
    {
        $request->validate([
            'nama_agama' => 'required',
        ]);
    ...
    public function hapusAgama82(Request $req)
    {
        Agama::destroy($req->id);
        return Redirect::back()->with('success', 'Agama berhasil dihapus');
    }

    public function updateAgama82(Request $request, $id)
    {
        $request->validate([
            'nama_agama' => 'required',
        ]);
    ...
    public function setIsAktif82(Request $req, $id)
    {
        $user = User::find($id);
    ...
    public function detailUser82($id)
    {
        $user = User::find($id);
    ...
    public function updateData82(Request $request)
    {
    ...
    public function register82(Request $req)
    {
        $req->validate([
            'name' => 'required',
    ...
    public function login82(Request $req)
    {
        $req->validate([
        ...
        if (!$user || !Hash::check($req->password, $user->password)) {
            ...
            return Redirect::to('/user/profile82');
        } else if ($user && $user->role == 2) {
            $allUser = User::all();
            Auth::login($user);
            $req->session()->put('admin', $user);
            $req->session()->put('users', $allUser);
            return Redirect::to('/admin/dashboard82');
        }
    }

    public function logout82()
    {
        if (session()->has('user')) {
            session()->pull('user');
        }
